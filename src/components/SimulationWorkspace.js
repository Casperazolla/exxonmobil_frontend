...
function EsdTab({ out }) {
  ...
  const payCell = (v, isCur) => {
    ...
    return <td key={v} style={{
      textAlign: 'center', background: bg, color: cl, fontWeight: 600, fontSize: 10,
      outline: isCur ? '2px solid #D97706' : 'none', outlineOffset: -2
    }}>{v?.toFixed(1)}y</td>;
  };
...
function CiiTab({ out }) {
  ...
  const buildChart = useCallback((id, data, extra = {}) => {
    ...
    chartsRef.current[id] = new window.Chart(canvas, {
      type: 'line', data,
      ...
      options: {
        ...
        scales: {
          ...
          y: {
            grid: { color: 'rgba(0,0,0,.04)', drawBorder: false }, ticks: { font: { size: 10 } },
...
function FinancialTab({ out }) {
  ...
  useEffect(() => {
    ...
    if (overviewRef.current) {
      ...
      chartsRef.current.overview = new window.Chart(overviewRef.current, {
        type: 'bar', data: {
    ...
    if (cashRef.current) {
      ...
      chartsRef.current.cash = new window.Chart(cashRef.current, {
        type: 'line', data: {
          datasets: [
            {
    ...
    if (opexRef.current) {
      ...
      chartsRef.current.opex = new window.Chart(opexRef.current, {
        type: 'bar', data: {
  ...
  return (
    <div className="rwrap">
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        <div className="card-hd">
          ...
          <span style={{ fontSize: 9, color: 'var(--purple)' }}>
            FuelEU grows from 2030
          </span>
        </div>

        <div className="card-body">
          <div className="ch h300">
            <canvas ref={opexRef}></canvas>
          </div>
        </div>
      </div>
      ...
      <div className="g2">
        ...
        {/* Accumulated Cashflow - unchanged */}
        <div className="card">
          <div className="card-hd">
            <span className="card-title">Accumulated Cashflow</span>
        ...
        {/* Investment Overview - moved here */}
        <div className="card">
          <div className="card-hd">
            <span className="card-title">
              Investment Overview — {out?.input?.vessel?.vessel_name || '—'}
            </span>
            <span style={{ fontSize: 9, color: 'var(--ink3)' }}>
              Total cost vs returns over vessel life
            </span>
          </div>
...
export default function SimulationWorkspace({ vesselId, vesselName, sessionMode, initialReport, vesselReports, onBack }) {
  ...
  const populateSidebar = (inp) => {
    ...
    setEsds(esdMeasures.map(e => {
      ...
      const def = iDef(cat);
      return {
        name: e.name, category: cat, efficiency_gain_percent: e.efficiency_gain_percent || 0,
